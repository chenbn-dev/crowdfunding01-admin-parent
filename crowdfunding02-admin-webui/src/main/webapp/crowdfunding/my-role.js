...
function fillAuthTree() {
    ...
    if (ajaxReturn.status != 200) {
        layer.mag("请求处理出错！响应状态码是：" + ajaxReturn.status + "说明是：" + ajaxReturn.statusText);
        return;
    }
    ...
    var setting = {
        "data": {
            "simpleData": {
                "enable": true,// 开启简单 JSON 功能
                "pIdKey": "categoryId"// 使用 categoryId 属性关联父节点，不用默认的 pId 了
            },
            "key": {
                "name": "title"// 使用 title 属性显示节点名称，不用默认的 name 作为属性名了
            }
        },
        "check": {
            "enable": true
